...
echo '<form method="post"><label for="use_img_post"><input type="checkbox" '; if (get_option ('use_img')) { echo 'CHECKED '; } echo 'name="use_img_post" value="use_img_post" onPropertyChange="showHide(\'imgsttngs\');" /> '. __('Convert sentenses to image', 'ptimathcaptcha').'</label><br /><div id="imgsttngs" style="display:'; if (get_option ('use_img')) { echo 'block'; } else { echo 'none';} echo '"><br />'. __('Image background color (RGB)', 'ptimathcaptcha').'<br />'.get_colors ($array, 'bg', 'bgColor = imagecolorallocate').'<br /><br />'. __('Text color in image (RGB)', 'ptimathcaptcha').'<br />'.get_colors ($array, 'im', 'textColor = imagecolorallocate').'</div><br /><input type="submit" name="submit" value="'. __('Submit', 'ptimathcaptcha').'" /></form>';
echo '
<script type="text/Javascript">
